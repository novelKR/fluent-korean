---
name: fluent-korean
description: "의미가 명확한 한국어 문장을 출력하는 작성 지침. 사용자가 한국어로 말하거나 답변, 보고, 문서처럼 한국어 결과물을 작성할 때 읽고 따른다. 코드, 코드 주석, 커밋 메시지, 로그 문자열, 인용문에는 적용하지 않는다. 코딩 작업에서는 이 skill을 사용하고 fluent-korean-not-coding은 사용하지 않는다."
license: MIT
metadata:
  author: snflkd
  short-description: "명확한 한국어 출력 (코딩)"
