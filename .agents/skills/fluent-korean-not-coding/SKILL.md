...
description: "코딩 지침을 제거한 명확한 한국어 작성 지침. 사용자가 /fluent-korean-not-coding을 명시적으로 요청할 때만 사용한다. 코드를 직접 고치지 않는 글쓰기에서만 켠다. 일반적인 코딩 대화와 한국어 응답에는 fluent-korean을 사용한다."
license: MIT
metadata:
  author: snflkd
  short-description: "명확한 한국어 출력 (코딩 지침 없음)"
